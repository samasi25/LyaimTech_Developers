...
import LoadingPage from "@/components/LoadingPage"; // Import custom loading page
...
const Contest = () => {
    ...
    const handleJoinContest = async (contestId, matchId) => {
        ...
        return (
            ...
            >
                {/* Only show Navbar when loading is done */}
                {!loading && <Navbar />}

                {/* Show the Loading Page if 'loading' is true */}
                {loading ? (
                    <LoadingPage /> // Display custom loading page while fetching data
                ) : (
