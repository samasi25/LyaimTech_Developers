...
export default function Wallet() {
    ...
    const handlePaymentSuccess = async (paypalToken) => {
        try {
            const response = await apiService.fetchData(`/wallet/add-funds/success?token=${paypalToken}`);
            ...
            <div className="w-full max-w-4xl mt-20 p-6 rounded-lg shadow-xl font-aleo text-black flex flex-col items-center">
                <div className="flex w-full md:gap-12 gap-7 items-center">
                    ...
                    <div className="flex flex-col space-y-4 w-full">
                        <h1 className="md:text-2xl text-xl font-semibold max-[400px]:text-sm">Hii, <span className="font-bold">Username</span> <span className="bg-[#0a0440] md:text-xl text-sm bg-opacity-25 text-[#0a0440] px-4 py-2 max-[400px]:px-1 rounded-lg">Total Money: <span className='text-white'> $ {wallet?.totalMoney?.toFixed(2) || '0.00'} </span></span></h1>
